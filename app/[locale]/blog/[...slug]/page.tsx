...
export default async function Page({ params: { slug, locale } }: BlogPageProps) {
  ...
  // Fix random ID generation at the end of the URL
  post.toc = post.toc.map((t) => {
    return {
      ...t,
      url: t.url.replace(/-\d+$/, "")
    }
  ...
  return (
    <>
      ...
      >
        <MDXLayoutRenderer code={post.body.code} components={components} toc={post.toc} locale={locale} />
      </Layout>
    </>
  )
}
