...
We have to modify the `store` method in `app/Http/Controllers/Auth/AuthenticatedSessionController.php` to return the authenticated user after login. The same applies to the `store` method in `app/Http/Controllers/Auth/RegisteredUserController.php` after registering an user.
