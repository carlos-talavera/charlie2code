...
Hay que modificar el método `store` en `app/Http/Controllers/Auth/AuthenticatedSessionController.php` para devolver el usuario autenticado al iniciar sesión. Lo mismo aplica para el método `store` en `app/Http/Controllers/Auth/RegisteredUserController.php` al registrar un usuario.
